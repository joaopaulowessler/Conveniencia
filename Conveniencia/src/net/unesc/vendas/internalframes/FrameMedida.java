...
import net.unesc.vendas.actions.ActionMedida;
...
public class FrameMedida extends javax.swing.JInternalFrame {

    private ActionMedida actMedida = new ActionMedida(this);
    
    public FrameMedida() {
        initComponents();
        
        salvar.addActionListener(actMedida);
        excluir.addActionListener(actMedida);
        cancelar.addActionListener(actMedida);
        novo.addActionListener(actMedida);
    }    
    
    public void novoMedida(){        
        nome.setText("");                
    ...
    private void codigoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_codigoFocusLost
        ...
        if(codTxt.length() > 0){
            ...
            Medida con = MedidaDao.getMedida(codigo);
            ...
            if(con != null){
                ...
                JOptionPane.showMessageDialog(null,"Unidade de Medida Não Encontrado!");
    ...
    private void setMedida(Medida con) {
        codigo.setText(String.valueOf(con.getMedCodigo()));
        nome.setText(con.getMedNome());        
    }
    
    public Medida getMedida()throws ExceptionVendas{
        Medida con = new Medida();
        
        if (codigo.getText().trim().isEmpty()){
            throw new ExceptionVendas("Código da Unidade de Medida deve ser informado!");       
        }
        
        if (!validaCaracteres(codigo.getText())) {
            throw new ExceptionVendas("Código da Unidade de Medida inválido!");
        }
        
        if (nome.getText().trim().isEmpty()){
            nome.requestFocus();
            throw new ExceptionVendas("Nome da Unidade de Medida deve ser informado!");
        }
        
        con.setMedCodigo(Integer.parseInt(codigo.getText()));
        con.setMedNome(nome.getText());        
